...
class TestChatAPI:
    ...
    def test_chat_returns_503_when_rag_unconfigured(
        self,
        client,
        mock_rag_unconfigured
    ):
        res = client.post(
            "/api/chat",
            data=json.dumps({"message": "What is diabetes?"}),
...
class TestPredictionAPI:
    ...
    def test_prediction_insufficient_data_returns_400(self, client):
        # No recognizable clinical fields at all ->
        # DiseasePredictor.predict_all raises ValueError
        data = {
            "favorite_color": "blue"
        }
...
class TestXrayAPI:
    ...
    def test_xray_invalid_base64(self, client):
        # Mock the analyzer to exercise the app-level status-code mapping
        # for code="invalid_input" without needing torch/trained weights.
    ...
    def test_xray_untrained_model_returns_200_with_available_false(
        self,
        client,
        mock_xray_untrained
    ):
        payload = {
            "image": f"data:image/png;base64,{_sample_image_b64()}"
        }
    ...
    def test_xray_success_with_mocked_trained_model(
        self,
        client,
        mock_xray_trained
    ):
    ...
    def test_xray_missing_model_does_not_crash_flask(self, client):
        # Simulate XRayAnalyzer() constructor itself raising
        # (e.g. bad checkpoint).
        # app.py's XRayAnalyzer already guards __init__ internally,
        # but this test confirms the /api/predict/xray route also
        # never 500s into a dead app.
        with patch(
            "app.XRayAnalyzer",
...
class TestIngestAPI:
    ...
    def test_ingest_missing_pdf_returns_400(self, client, mock_rag):
        res = client.post(
            "/api/ingest",
            data=json.dumps({
                "pdf_path": "data/does_not_exist.pdf"
            }),
            content_type="application/json"
        )
    ...
    def test_ingest_default_pdf_path_is_medical_book(
        self,
        client,
        mock_rag
    ):
        # The medical PDF is intentionally not committed to GitHub.
        # Mock its existence so this test validates the API behavior.
    ...
    def test_ingest_returns_503_when_rag_unconfigured(
        self,
        client,
        mock_rag_unconfigured
    ):
        # Mock the PDF as present so the test reaches RAG initialization.
        with patch("app.os.path.exists", return_value=True):
            res = client.post(
                "/api/ingest",
                data=json.dumps({}),
